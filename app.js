const YR_FINANCAS_VERSION = 'v25.12.2-fechamento-fatura';
...
(() => {
  ...
  const DASHBOARD_MONTH_KEY = 'yr-financas-dashboard-month';
  const today = new Date();
  const currentMonth = today.getMonth() + 1;
  const currentYear = today.getFullYear();

  function getRememberedDashboardMonth() {
    try {
      const savedMonth = localStorage.getItem(DASHBOARD_MONTH_KEY);
      const month = Number(savedMonth);
  ...
  function rememberDashboardMonth(month) {
    const parsed = Number(month);
    if (!Number.isFinite(parsed) || parsed < 1 || parsed > 12) return;
    try {
      localStorage.setItem(DASHBOARD_MONTH_KEY, String(parsed));
    } catch (error) {}
  }
  ...
  const state = {
    ...
    charts: {},
    filters: {
      dashboard: { month: getRememberedDashboardMonth(), year: currentYear, cardId: 'all' },
  ...
  function getInvoiceStartDate(purchase) {
    const dateText = purchase?.date;
    const [y, m, d] = String(dateText || '').split('-').map(Number);
    if (!y || !m) return new Date(currentYear, currentMonth - 1, 1, 12, 0, 0);

    const paymentMethod = purchase.paymentMethod || 'Cartão de crédito';
    if (paymentMethod !== 'Cartão de crédito' || !purchase.cardId) {
      return new Date(y, m - 1, d || 1, 12, 0, 0);
    }

    const card = getById(state.data?.cards || [], purchase.cardId);
    const closeDay = Number(card?.closeDay || 0);

    // Regra de fatura:
    // Se a compra foi depois do dia de fechamento, ela entra na fatura do mês seguinte.
    // Ex.: cartão fecha dia 3; compra dia 10/06 => fatura de julho.
    if (closeDay >= 1 && closeDay <= 31 && Number(d || 1) > closeDay) {
      return new Date(y, m, 1, 12, 0, 0);
    }
  ...
  function generateInstallments(purchase) {
    ...
    const firstInvoiceDate = getInvoiceStartDate(purchase);

    return values.map((value, index) => {
      const ref = addMonths(`${firstInvoiceDate.getFullYear()}-${String(firstInvoiceDate.getMonth() + 1).padStart(2, '0')}-01`, index);
      return {
  ...
  function repairInstallmentsFromPurchases() {
    ...
    state.data.purchases.forEach(purchase => {
      ...
      const expectedInstallments = generateInstallments(purchase);
      const wrongInvoiceMonth = current.some((item, index) => {
        const expected = expectedInstallments[index];
        return !expected || Number(item.month) !== Number(expected.month) || Number(item.year) !== Number(expected.year);
      });
      ...
      if (needsRepair) {
        ...
        state.data.installments.push(...expectedInstallments);
      }
    });
...
`;
  ...
  function ensureAuthGate() {
    ...
    gate.innerHTML = `
      <div class="auth-card">
        <div class="brand auth-brand"><div class="brand-logo"><img src="icon-192.png?v=2512fix2" alt="Logo YR Finanças"></div><div><strong>YR Finanças</strong><span>sincronização em nuvem</span></div></div>
        <div class="auth-copy">
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('./sw.js?v=2512fix2').catch((error) => {
